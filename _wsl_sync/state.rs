...
use vsbmas_round::{MultiRoundStatus, RoundManager};
...
pub struct MiningSnapshot {
    pub session_top_id: u32,
    pub riggs_auction_id: u32,
    ...
    pub empty_round: bool,
    pub rankings: BidRankings,
    /// 来自 `revealed_rankings`，与投标者对齐。
    pub revealed_by_uid: Vec<(u32, u32)>,
    pub round_number: u32,
    pub streak_snap: u32,
    pub round_status_snapshot: MultiRoundStatus,
}
...
pub struct AppState {
    ...
    /// 真实 VSBMAS 每一轮至多一个区块：(session_top_id, riggs_auction_id) -> DemoBlockchain index。
    pub mined_round_blocks: Arc<Mutex<HashMap<(u32, u32), u64>>>,
    /// advance_round / 手动补挖失败后保留的快照（key = riggs_auction_id）。
    pub mining_retry_snapshots: Arc<Mutex<HashMap<u32, MiningSnapshot>>>,
}
...
impl AppState {
    pub async fn new() -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            mined_round_blocks: Arc::new(Mutex::new(HashMap::new())),
            mining_retry_snapshots: Arc::new(Mutex::new(HashMap::new())),
        })
    }
}
