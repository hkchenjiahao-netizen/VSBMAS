...
mod tests {
    ...
    /// Part B1 负例：对承诺 `C_1` 生成的 range proof 不得在另一承诺 `C_2` 上通过验证。
    ///
    /// 对应攻击场景：攻击者持有合法 bid=1000 的 range proof，尝试"移植"到不同承诺
    /// （例如伪造 bid=reserve-1 但声称 bid>=reserve）上蒙混过验。Bulletproofs 的
    /// Fiat-Shamir 挑战将 proof 与承诺绑定，跨承诺复用必定失败。
    #[test]
    fn bulletproofs_rejects_proof_rebinding_to_foreign_commit() {
        let mut rng = StdRng::seed_from_u64(11u64);
        let ped_pp = PedersenComm::<G>::gen_pedersen_params(&mut rng);
        let pp = Bulletproofs::<G, Sha3_256>::gen_params(&mut rng, 32);

        let v1 = BigInt::from(1000u32);
        let (comm1, opening1) =
            PedersenComm::<G>::commit(&mut rng, &ped_pp, &v1.to_bytes_le().1).unwrap();
        let proof = Bulletproofs::<G, Sha3_256>::prove_range(
            &mut rng, &pp, &ped_pp, &comm1, &v1, &opening1, 32,
        )
        .unwrap();
        // 原承诺上验证通过
        assert!(
            Bulletproofs::<G, Sha3_256>::verify_range(&pp, &ped_pp, &comm1, 32, &proof).unwrap()
        );

        // 另一不同承诺上验证必拒
        let v2 = BigInt::from(500u32);
        let (comm2, _opening2) =
            PedersenComm::<G>::commit(&mut rng, &ped_pp, &v2.to_bytes_le().1).unwrap();
        let ok = Bulletproofs::<G, Sha3_256>::verify_range(&pp, &ped_pp, &comm2, 32, &proof)
            .unwrap_or(false);
        assert!(
            !ok,
            "range proof bound to commit C_1 must not verify against a different commit C_2"
        );
    }

    /// Part B1 负例：对"承诺位移 `C - g·k`"的 range proof 在改变下界 `k` 后必须失效。
    ///
    /// 对应 `prove_bid_at_least(bid, lower_bound=k)` 的 verifier gating：证明与 `k`
    /// 绑定到同一 shifted 承诺；若 verifier 用不同 `k'` 重构 shifted 承诺，点位不同，
    /// proof 不可能通过。这正是"恶意 bid = reserve - 1 无法生成通过验证的证明"的
    /// 数学保证。
    #[test]
    fn bulletproofs_rejects_shifted_proof_across_lower_bound() {
        let mut rng = StdRng::seed_from_u64(29u64);
        let ped_pp = PedersenComm::<G>::gen_pedersen_params(&mut rng);
        let pp = Bulletproofs::<G, Sha3_256>::gen_params(&mut rng, 32);

        // 真实 bid = 500；合法下界 k1 = 300 （证明 bid >= 300，即 bid-300=200 在 [0,2^32)）
        let bid = 500u32;
        let k1 = 300u32;
        let k2 = 600u32; // > bid；即便攻击者声称 bid >= 600，verifier 必拒

        let v_bid = BigInt::from(bid);
        let (comm_bid, opening) =
            PedersenComm::<G>::commit(&mut rng, &ped_pp, &v_bid.to_bytes_le().1).unwrap();

        // 构造对 k1 的合法 shifted 承诺与 proof
        let f_k1 = <G as ProjectiveCurve>::ScalarField::from(k1 as u64);
        let shifted_k1 = comm_bid.clone() - ped_pp.g.mul(&f_k1.into_repr());
        let proof_k1 = Bulletproofs::<G, Sha3_256>::prove_range(
            &mut rng,
            &pp,
            &ped_pp,
            &shifted_k1,
        ...
        .unwrap();
        assert!(
            Bulletproofs::<G, Sha3_256>::verify_range(&pp, &ped_pp, &shifted_k1, 32, &proof_k1)
                .unwrap()
        );

        // verifier 若用不同 k2 重构 shifted 承诺，proof 必失效
        let f_k2 = <G as ProjectiveCurve>::ScalarField::from(k2 as u64);
        let shifted_k2 = comm_bid - ped_pp.g.mul(&f_k2.into_repr());
        let ok = Bulletproofs::<G, Sha3_256>::verify_range(&pp, &ped_pp, &shifted_k2, 32, &proof_k1)
            .unwrap_or(false);
        assert!(
            !ok,
            "shifted-commitment range proof must not verify across different lower bounds"
        );
    }
}
